      ...
      <nav className="relative z-20 flex items-center justify-between p-6 lg:p-8">
        ...
        <ul className="hidden md:flex items-center space-x-8 text-sm font-medium text-gray-300">
          {['Home', 'Work', 'Services', 'Experience', 'Contact'].map((item) => (
            <li key={item} className="hover:text-cyan-400 cursor-pointer transition-colors">
              {item}
            </li>
          ))}
        </ul>
        ...
        <div className="container mx-auto px-8 lg:px-16 max-w-7xl">
          ...
          >
            ...
            <p className="text-gray-300 mb-8 leading-relaxed">
              Independent composer & full-stack software engineer. 100% ownership of writing,
              publishing, and master rights. Tier-1 Music Supervisor with encrypted 7-stem delivery,
              zero legal friction, undeniable sound.
            </p>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              {[
                ...
                <div
                  key={idx}
                  className={`relative p-5 rounded-xl bg-black/60 backdrop-blur-sm border-2 overflow-hidden flex flex-col justify-between ${
                    stat.color === 'cyan'
                      ? 'border-cyan-400/60 shadow-[0_0_15px_rgba(0,255,255,0.2)]'
                ...
                >
                  ...
                  <p
                    className={`text-4xl font-black z-10 ${
                      stat.color === 'cyan' ? 'text-cyan-300' : 'text-yellow-400'
                    }`}
                  >
                    {stat.value}
                  </p>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2 text-xs font-mono">
                {['APEX', 'GOD-OF-ALL-GODS', 'MASTER GRADE', 'TIER-1 SECURE'].map((tag) => (
                  <span
                    key={tag}
                    className="border border-cyan-400/30 bg-cyan-400/10 px-3 py-1 rounded text-cyan-300"
                  >
                    {tag}
                  </span>
                ))}
